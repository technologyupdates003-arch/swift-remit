...
const MpesaWithdrawForm = ({ wallet, isOpen, onClose, onSuccess }: MpesaWithdrawFormProps) => {
  ...
  const validateInputs = () => {
    ...
    if (parseFloat(amount) < 10) return 'Minimum withdrawal amount is KSh 10';
    ...
    if (!phoneNumber.match(/^254[17][0-9]{8}$/)) return 'Invalid M-Pesa number format. Use 254XXXXXXXXX';
  ...
  const handleWithdrawal = async () => {
    ...
    try {
      // Call Daraja B2C edge function directly
      const { data: b2cResult, error: b2cError } = await supabase.functions.invoke('daraja-b2c', {
        body: {
          phone_number: phoneNumber,
          amount: parseFloat(amount),
          wallet_id: wallet.id,
          user_id: user.id,
          narrative: `Withdrawal to ${recipientName.trim()}`,
        },
      });
  ...
  const getCurrencySymbol = (currency: string) => (currency === 'KES' ? 'KSh' : currency);
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Send money from your wallet to any M-Pesa number</DialogDescription>
        </DialogHeader>
        ...
        {!isMpesaSupported ? (
          <div className="space-y-4 pt-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                M-Pesa withdrawals are only available for KES wallets. Current wallet currency: {wallet.currency}
              </p>
            </div>
          ...
          <div className="space-y-6 pt-4">
            <div className="bg-muted p-3 rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">Wallet: {wallet.wallet_number}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                maxLength={50}
              />
              <p className="text-xs text-muted-foreground">Name as registered with Safaricom</p>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Secured by Safaricom Daraja & M-Pesa</span>
            </div>
          </div>
        ) : step === 'processing' ? (
          <div className="space-y-6 pt-4">
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">Your M-Pesa withdrawal is being processed</p>
          ...
          <div className="space-y-6 pt-4">
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold text-green-900 mb-2">Withdrawal Successful!</h3>
